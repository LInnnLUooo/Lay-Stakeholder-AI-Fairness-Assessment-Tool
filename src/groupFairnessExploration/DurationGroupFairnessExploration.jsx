...
function ImageSection({selectedGroupFairnessMetric}) {
  const Combinations = 
  featureCombinations_numeric_allvalues[featureName];
  const uniqueValues = uniqueNumericValues[featureName];
  ...
  const calculateMaxHeight = (combinations) => {
    ...
    return Math.max(...Object.values(combinations).map(ids => {
      if (!ids) return 0;
      return Math.ceil(ids.length / 5) * (imageHeight + idHeight + 10); // 每行5个图片，每个图片和ID的总高度加上间距
    }));
  };
  ...
  return (
    <Box display="flex" flexDirection="row" justifyContent="space-evenly" padding="3px">
      {uniqueValues.map((featureValue, index) => {
        const imagesPredGoodRealGood = mapImages(Combinations[`group_predGood_realGood_${featureName.toLowerCase().replace(/ /g, '')}_${featureValue}`], 'Good');
        const imagesPredGoodRealBad = mapImages(Combinations[`group_predGood_realBad_${featureName.toLowerCase().replace(/ /g, '')}_${featureValue}`], 'Bad');
        const imagesPredBadRealGood = mapImages(Combinations[`group_predBad_realGood_${featureName.toLowerCase().replace(/ /g, '')}_${featureValue}`], 'Good');
        const imagesPredBadRealBad = mapImages(Combinations[`group_predBad_realBad_${featureName.toLowerCase().replace(/ /g, '')}_${featureValue}`], 'Bad');
        ...
        return (
          <Box
            ...
            flexGrow={1}
          >
            <h2>{featureName}: {featureValue}</h2>
...
if (feature === 'Property') {
  ...
  return (
    <Box sx={{ width: '98%', height: '100%',margin: '20px auto' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '20px',height: '100%'}}>
       ...
       I am interested in this trait: Under the same conditions of
        ...
        <FormControl sx={{ marginLeft: '10px', minWidth: '180px', flexGrow: 1 }}>
...
export default function DurationGroupFairnessExploration(){
  ...
  const calculateMetricValues = (metric) => {
    const values = uniqueValues.map(value => {
      const imagesPredGoodRealGood = Combinations[`group_predGood_realGood_${featureName.toLowerCase().replace(/ /g, '')}_${value}`]?.length || 0;
      const imagesPredGoodRealBad = Combinations[`group_predGood_realBad_${featureName.toLowerCase().replace(/ /g, '')}_${value}`]?.length || 0;
      const imagesPredBadRealGood = Combinations[`group_predBad_realGood_${featureName.toLowerCase().replace(/ /g, '')}_${value}`]?.length || 0;
      const imagesPredBadRealBad = Combinations[`group_predBad_realBad_${featureName.toLowerCase().replace(/ /g, '')}_${value}`]?.length || 0;
