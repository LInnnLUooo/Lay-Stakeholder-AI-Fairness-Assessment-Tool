...
function ImageSection({selectedGroupFairnessMetric}) {
    ...
    const calculateMaxHeight = (combinations) => {
      ...
      return Math.max(...Object.values(combinations).map(ids => {
        if (!ids) return 0;
        return Math.ceil(ids.length / 10) * (imageHeight + idHeight + 16); // 每行5个图片，每个图片和ID的总高度加上间距
      }));
    };
...
if (feature === 'Property') {
  ...
  return (
    ...
    justifyContent: 'flex-start', marginBottom: '20px',height: '100%'}}>
        <Typography variant="h6" sx={{ flexShrink: 0,textAlign: 'left'}}>
...
export default function AgeGroupFairnessExploration(){
