...
const featureName = 'Installment Rate';
...
export default function InstallmentRateGroupFairnessExploration(){
    ...
    const getDescriptionForMetric = (metric) => {
      // 添加对"Conditional Statistical Parity"的检查
      if (metric === "Conditional Statistical Parity") {
        return null; // 不显示任何内容
      }
      const values = uniqueValues.map(value => {
        const encodedFeatureValue = encodeURIComponent(value.toString().toLowerCase().replace(/ /g, ''));
