...
function ImageSection({selectedGroupFairnessMetric}) {
    ...
    const calculateMaxHeight = (combinations) => {
      ...
      return Math.max(...Object.values(combinations).map(ids => {
        if (!ids) return 0;
        return Math.ceil(ids.length / 10) * (imageHeight + idHeight + 12); // 每行5个图片，每个图片和ID的总高度加上间距
      }));
    };
...
export default function DependentsGroupFairnessExploration(){
