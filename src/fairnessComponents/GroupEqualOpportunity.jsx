...
//nevigate path：
const EOIMAGE_AGE_PATH = "/explanationAge/EOimage";

export default function GroupEqualOpportunity() {
    const navigate = useNavigate(); 
    // Scatter Plot 
    const [data] = useState([93,97,-4])
    const svgRef = useRef();
    ...
    // svg.append('text').attr('x',w/3).attr('y',h+10).text('Equal Opportunity'); 已经设置好位置了可以直接使用的，但是不需要，因为已经有单独的标题了。
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center" width = {500}> 
        ...
        <Box display="flex" justifyContent="space-between" width="100%">
            ...
            <Tooltip 
                title={
                    ...
                    >
                    The AI application is fairness if it has "equal" probability for Rated Good Credit Age{'<'}25 Individuals and Rated Good Credit Age{'>='}25 Individuals to have good predicted credit.
                    </span>
                } 
                placement="left"
                >    
        ...
        <Typography variant="body2" color="text.secondary" sx={{ margin: "12px",color: "black", fontWeight: "bold"}}>
           Of the <span style={{textDecoration: 'underline' }}>age{"<"}25 with Rated Good Credit</span>, <span style={{ color: '#1976D2' }}>93%</span> are predicted as <span style={{textDecoration: 'underline' }}>Good Credit</span>.
            <br />
            Of the <span style={{textDecoration: 'underline' }}>age{">="}25 with Rated Good Credit</span>, <span style={{ color: '#1976D2' }}>97%</span> are predicted as <span style={{textDecoration: 'underline' }}>Good Credit</span>.
            <br />
            The difference between the age{"<"}25 and the age{">="}25 is <span style={{ color: '#1976D2' }}>-4%</span>.
        </Typography>
        ...
        <Box 
            display="flex"
            alignItems="center"  > 
             <Button variant="contained" size="small" startIcon={<TouchAppIcon />} onClick={() => navigate(EOIMAGE_AGE_PATH)}>Check individual instances</Button>
        </Box>  
