...
//nevigate path：
const EOIMAGE_GENDER_PATH = "/explanationGender/EOimage";

export default function GroupEqualOpportunityGender() {
    const navigate = useNavigate(); 
    
    // Scatter Plot 
    const [data] = useState([87,99,-12])
    const svgRef = useRef();
    ...
    svg.append('g').call(xAxis).attr('transform', `translate(0, ${h/3*2})`).style("font-size", "18px").selectAll("text").attr("dy", "40px");// 将一个<g>元素添加到SVG元素中。<g>元素通常用于组合其他图形元素，例如坐标轴、标签等，以便进行位置和样式的集中管理
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center" width = {500}> 
        ...
        <Box display="flex" justifyContent="space-between" width="100%">
            ...
            <Tooltip 
                title={
                    ...
                    >
                    The AI application is fairness if it has "equal" probability for Rated Good Credit females and Rated Good Credit males to have good predicted credit.
                    </span>
                } 
                placement="left"
                >    
        ...
        <Typography variant="body2" color="text.secondary" sx={{ margin: "12px",color: "black", fontWeight: "bold"}}>
            Of <span style={{textDecoration: 'underline' }}>females with Rated Good Credit</span>, <span style={{ color: '#1976D2' }}>87%</span> are predicted as <span style={{textDecoration: 'underline' }}>Good Credit</span>.
            <br />
            Of <span style={{textDecoration: 'underline' }}>males with Rated Good Credit</span>, <span style={{ color: '#1976D2' }}>99%</span> are predicted as <span style={{textDecoration: 'underline' }}>Good Credit</span>.
            <br />
        ...
        display="flex"
        alignItems="center"  > 
            <Button startIcon={<TouchAppIcon />} variant="contained" onClick={() => navigate(EOIMAGE_GENDER_PATH)} size="small">Check individual instances</Button>
        </Box>
    </Box>     
