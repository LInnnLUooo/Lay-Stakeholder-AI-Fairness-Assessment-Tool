...
//nevigate path：
const DPIMAGE_GENDER_PATH = "/explanationGender/DPimage";

export default function GroupDemographicParity() {
    const navigate = useNavigate(); 

    // Scatter Plot 
    const [data] = useState([81,92,-11])
    const svgRef = useRef();
    ...
    .text(d =>`${d}%`); // display the data value, 这里一定要加“d=> ”因为与female 和male bar 不同，这里没有单独写成一个function（d）的形式。
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center" width = {500}> 
        ...
        <Box display="flex" justifyContent="space-between" width="100%">
            ...
            <Tooltip 
                title={
                    ...
                    >
                    The AI application is fairness if it has "equal" probability for females and males to have good predicted credit.
                    </span>
                } 
                placement="left"
                >    
        ...
        <Typography variant="body2" color="text.secondary" sx={{ margin: "12px",color: "black", fontWeight: "bold"}}>
           Of the <span style={{textDecoration: 'underline' }}>total females</span>, <span style={{ color: '#1976D2' }}>81%</span> are predicted as <span style={{textDecoration: 'underline' }}>Good Credit</span>.
            <br />
            Of the <span style={{textDecoration: 'underline' }}>total males</span>, <span style={{ color: '#1976D2' }}>92%</span> are predicted as <span style={{textDecoration: 'underline' }}>Good Credit</span>.
            <br />
        ...
        alignItems="center" 
        marginTop='20px' > 
            <Button startIcon={<TouchAppIcon />} variant="contained" size="small" onClick={() => navigate(DPIMAGE_GENDER_PATH)} >Check individual instances</Button>
        </Box>
